...
% read image
im = imread(['datasets/Test/Image/', num2str(num,'%06.f') ,'.png']);
% read results in mat files (predicted points)
load(['results/test', num2str(num,'%06.f') ,'.mat']);

% read json files (target points and their label)
fname = ['datasets/Test/Point_Location/', num2str(num, '%06.f'),'.json'];
val = jsondecode(fileread(fname));
target_points = [size(im,1)-val.Y, val.X];

fname = ['datasets/Test/Coarse_Label/', num2str(num, '%06.f'),'.json'];
val = jsondecode(fileread(fname));
target_labels = val;
